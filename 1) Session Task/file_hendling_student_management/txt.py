...
def insert_roll_no(pera):
    ...
    if roll not in roll_list:
        if roll.isdigit() == True:
            with open("data.txt",pera) as file1:
                file1.write(f"{roll},")
        else:
...
def insert_name(pera):
    try:
        ...
        if name.isalpha() == True:
            with open("data.txt",pera) as file2:
                file2.write(f"{name},")
        else:
...
def insert_department(pera):
    ...
    with open("data.txt",pera) as file3:
        file3.write(f"{department},")
# insert_department("a")

def insert_city(pera):
    city = input("Enter City : ")
    with open("data.txt",pera) as file4:
        file4.write(f"{city},\n")
# insert_city("a")
...
def delete_student2():
    ...
    with open("data.txt","r") as file:
        ...
        print(file_data)
        with open("data.txt","w") as delete_data:
            for i in file_data:
                if i[0] != delete_roll:
    ...
    with open("data.txt", "r") as file:
        ...
        for i in file_data:
            if i[0] == student_roll:
                ...
                count = count+1
        # print(file_data)
        with open("data.txt","w") as update_data:
            for j in file_data:
                    update_data.write(j)
