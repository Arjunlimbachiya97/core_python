...
def add_student_1():
    ...
    def roll_validation():
        ...
        # if student["Roll No"].isalpha() == True:
...
def display_student_by_Roll_No_6(name):
    ...
    if c == 1:
        ...
        json_object = json.dumps(all, indent=4)
        with open("ASC.json", "w") as outfile:
            outfile.write(json_object)
    elif c == 2:
...
def main():
    global all, student
    all = [{'Roll No': "1", 'Name': 'Arjun', 'Department': 'mca', 'Address': 'bhabhar'},
           {'Roll No': "2", 'Name': 'vishal', 'Department': 'b.com', 'Address': 'deesa'},
           {'Roll No': "3", 'Name': 'Ronak', 'Department': 'bca', 'Address': 'Rajcot'}]
    student = {}
    json_object = json.dumps(all, indent=4)
    with open("sample.json", "w") as outfile:
        outfile.write(json_object)
